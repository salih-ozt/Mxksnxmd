/*
 * =============================================================================
 * AGROLINK SERVER - SECURITY v5.0 (SIKILAŞTIRILMIŞ GÜVENLİK)
 * =============================================================================
 * 
 * 🔒 GÜVENLİK DEĞİŞİKLİKLERİ (v5.0):
 * 
 * ✅ 1. HARD-CODED SECRETS KALDIRILDI:
 *    - Gmail şifresi env vars'a taşındı (SMTP_USER, SMTP_PASS)
 *    - JWT_SECRET env vars'dan okunuyor
 *    - API_ENCRYPTION_KEY env vars'dan okunuyor
 * 
 * ✅ 2. SQL INJECTION & XSS KORUMASI AÇILDI:
 *    - enableSqlInjectionProtection: true
 *    - enableXssProtection: true
 *    - enableBotDetection: true
 * 
 * ✅ 3. TÜM HTTP METODLARINDA GÜVENLİK KONTROLÜ:
 *    - GET/HEAD/OPTIONS bypass KALDIRILDI
 *    - Tüm isteklerde tehdit analizi yapılıyor
 *    - Query parameters dahil SQL injection kontrolü
 * 
 * ✅ 4. HELMET SIKILAŞTIRILDI:
 *    - CSP server-side uygulandı
 *    - HSTS 1 yıl (preload dahil)
 *    - X-Frame-Options: DENY
 *    - Referrer-Policy: strict-origin-when-cross-origin
 * 
 * ✅ 5. GÜVENLİ COOKIE AYARLARI:
 *    - HttpOnly: true
 *    - Secure: true (production)
 *    - SameSite: strict
 * 
 * ✅ 6. CORS SIKILAŞTIRILDI:
 *    - Production'da sadece HTTPS origin'ler
 *    - HTTP IP adresleri kaldırıldı
 * 
 * ✅ 7. RATE LIMITING SIKILAŞTIRILDI:
 *    - maxRequestsPerSecond: 50 (2000'den düşürüldü)
 *    - maxRequestsPerMinute: 500 (20000'den düşürüldü)
 *    - banDurationMinutes: 30 (5'ten artırıldı)
 * 
 * ✅ 8. REQUEST SIZE LIMIT DÜŞÜRÜLDÜ:
 *    - 50mb -> 10mb
 * 
 * ⚠️ YAPILAMAYAN/EKSİK KALANLAR (manuel gerekli):
 *    - Redis TLS yapılandırması (hosting provider'a bağlı)
 *    - WAF entegrasyonu (Cloudflare/AWS WAF önerilir)
 *    - IDS/IPS sistemi (harici servis gerekli)
 *    - E-posta SPF/DKIM/DMARC (DNS ayarları gerekli)
 *    - 2FA/MFA zorunlu kılma (iş mantığı kararı)
 * 
 * =============================================================================
 */
...
// Gmail SMTP Transporter - 🔒 GÜVENLİK: Secrets env vars'dan okunuyor
// ÖNEMLİ: Bu değerleri .env dosyasına veya secret manager'a ekleyin:
// SMTP_USER=your-email@gmail.com
// SMTP_PASS=your-app-password
// SMTP_SERVICE=gmail (veya başka bir servis)
if (!process.env.SMTP_USER || !process.env.SMTP_PASS) {
    console.warn('⚠️ UYARI: SMTP_USER ve SMTP_PASS environment variables ayarlanmamış!');
    console.warn('E-posta gönderimi çalışmayacak. Lütfen aşağıdaki env vars\'ları ayarlayın:');
...
// Güvenlik duvarı konfigürasyonu - 🔒 GÜVENLİK v5.0 (SIKILAŞTIRILDI)
// ⚠️ ÖNCEKİ VERSİYON GÜVENLİK AÇIKLARI DÜZELTİLDİ
const FIREWALL_CONFIG = {
    maxRequestsPerSecond: 50,            // 🔒 Düşürüldü - DDoS koruması
    maxRequestsPerMinute: 500,           // 🔒 Düşürüldü - abuse önleme
    maxFailedLoginsPerHour: 10,          // 🔒 Brute force koruması
    suspiciousThreshold: 100,            // 🔒 Şüpheli aktivite eşiği
    banDurationMinutes: 30,              // 🔒 Ban süresi artırıldı
    permanentBanThreshold: 500,          // 🔒 Kalıcı ban eşiği
    enableSqlInjectionProtection: true,  // 🔒 AÇILDI - SQL Injection koruması ZORUNLU
    enableXssProtection: true,           // 🔒 AÇILDI - XSS koruması ZORUNLU
    enablePathTraversalProtection: true, // 🔒 Path traversal koruması
    enableBotDetection: true,            // 🔒 AÇILDI - Kötü amaçlı bot tespiti
    trustedProxies: ['127.0.0.1', '::1'], // 🔒 Sadece localhost trusted
    // 🔒 GÜVENLİK: Sadece gerçekten güvenli path'lerde atla
    skipSecurityForPaths: ['/api/health', '/api/ping'],
    skipSecurityForMethods: [] // 🔒 HİÇBİR METHOD'DA GÜVENLİK ATLANMAZ - Tüm isteklerde kontrol yapılır
};
...
// ==================== EXPRESS MIDDLEWARE - 🔒 GÜVENLİK v5.0 ====================

// 🔒 Helmet - Sıkılaştırılmış güvenlik başlıkları
app.use(helmet({
    // 🔒 CSP - Content Security Policy (server-side)
    contentSecurityPolicy: {
        directives: {
            defaultSrc: ["'self'"],
            scriptSrc: ["'self'", "https://cdn.tailwindcss.com", "https://unpkg.com", "https://cdnjs.cloudflare.com", "https://cdn.socket.io"],
            styleSrc: ["'self'", "https://cdnjs.cloudflare.com", "https://fonts.googleapis.com", "'unsafe-inline'"], // style için inline gerekli olabilir
    ...
    // 🔒 HSTS - HTTP Strict Transport Security (HTTPS zorunlu)
    ...
    // 🔒 X-XSS-Protection
    xssFilter: true,
    // 🔒 Cross-Origin ayarları
    crossOriginEmbedderPolicy: true,
    crossOriginOpenerPolicy: { policy: 'same-origin' },
    crossOriginResourcePolicy: { policy: "same-site" } // 🔒 Sıkılaştırıldı
}));
...
// 🔒 CORS - Sadece güvenilen origin'ler
app.use(cors({
    origin: function(origin, callback) {
        const allowedOrigins = [
            'https://sehitumitkestitarimmtal.com',
            'http://localhost:3000',  // Sadece development için
            'http://localhost:5173'   // Sadece development için
        ];
        
        // Production'da sadece HTTPS origin'lere izin ver
        if (process.env.NODE_ENV === 'production') {
            if (!origin || allowedOrigins.filter(o => o.startsWith('https')).includes(origin)) {
                callback(null, true);
            } else {
                callback(new Error('CORS policy violation'));
            }
        } else {
            // Development'ta tüm izin verilen origin'ler
...
app.use(express.json({ limit: '10mb' })); // 🔒 Limit düşürüldü (50mb -> 10mb)
app.use(express.urlencoded({ extended: true, limit: '10mb' })); // 🔒 Limit düşürüldü
...
app.use(async (req, res, next) => {
    ...
    // 🔒 GÜVENLİK: GET/HEAD/OPTIONS dahil TÜM isteklerde kontrol yapılır
    // ÖNCEKİ AÇIK: GET isteklerinde güvenlik atlanıyordu - bu SQL injection'a açık kapı bırakıyordu
    // Artık skipSecurityForMethods boş, bu blok çalışmayacak ama kod bütünlüğü için bırakıldı
    if (FIREWALL_CONFIG.skipSecurityForMethods.length > 0 && FIREWALL_CONFIG.skipSecurityForMethods.includes(req.method)) {
        // Sadece gerçekten güvenli olduğundan emin olunan durumlarda
        console.warn(`⚠️ Güvenlik kontrolü atlandı: ${req.method} ${req.path}`);
        return next();
    }
    ...
    // 🔒 GÜVENLİK: Tüm HTTP metodlarında rate limiting (GET dahil)
    // ÖNCEKİ AÇIK: Sadece POST/PUT/PATCH/DELETE kontrol ediliyordu
    ...
    // 🔒 GÜVENLİK: Tehdit analizi TÜM metodlarda (GET dahil - SQL injection query params'ta olabilir)
    ...
    // 🔒 Body sanitizasyonu - tüm isteklerde (POST, PUT, PATCH)
