...
// Tanımlı değilse in-memory (her worker bağımsız — production'da Redis önerilir).
// ════════════════════════════════════════════════════════════════════
let rateLimitStore = undefined; // undefined → express-rate-limit default (in-memory)
...
function makeRateLimitOptions(opts) {
    return rateLimitStore ? { ...opts, store: rateLimitStore } : opts;
}
...
// 🔒 Admin endpoint'leri için özel rate limiter (brute-force & scanning koruması)
const adminLimiter = rateLimit(makeRateLimitOptions({
    windowMs : 15 * 60 * 1000, // 15 dakika
    max      : 30,             // Admin başına 30 istek (normal kullanıcıdan çok daha az)
    standardHeaders: true,
    legacyHeaders  : false,
