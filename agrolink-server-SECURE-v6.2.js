...
const DB_POOL_CONFIG = {
    maxConnections: parseInt(process.env.DB_MAX_CONNECTIONS) || 50,  // Maksimum bağlantı sayısı
...
// 🚀 Cache Instances
const userCache = new MemoryCache(5000, 300000);      // 5dk kullanıcı cache
const postCache = new MemoryCache(2000, 180000);      // 3dk post cache
const feedCache = new MemoryCache(1000, 60000);       // 1dk feed cache
...
const requestQueue = new RequestQueue(200);
...
// 🔒 XSS Sanitization - HTML tehlikeli karakterleri escape et
...
// 🔒 User Input Sanitization - Genel kullanıcı girdisi temizleme
function sanitizeUserInput(value, options = {}) {
    ...
    // Control karakterleri kaldır (NULL byte, vb.)
    if (removeControlChars) {
        sanitized = sanitized.replace(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/g, '');
    }
    
    // HTML escape (allowHtml false ise)
    if (!allowHtml) {
...
function sanitizeUrl(url) {
    ...
    // Javascript protocol engelle
    if (trimmed.match(/javascript:/i)) {
        return null;
    }
    
    // Data URI engelle
    if (trimmed.match(/^data:/i)) {
        return null;
    }
...
const JWT_SECRET = process.env.JWT_SECRET || (() => {
    ...
    // Development fallback - random her başlatmada
    const devSecret = require('crypto').randomBytes(64).toString('hex');
    console.warn('⚠️ WARNING: Using random JWT secret for development. Set JWT_SECRET env variable!');
    console.warn(`⚠️ Generated secret: ${devSecret.substring(0, 20)}...`);
    return devSecret;
})();
const JWT_REFRESH_SECRET = process.env.JWT_REFRESH_SECRET || (() => {
    if (process.env.NODE_ENV === 'production') {
        console.error('❌❌❌ FATAL ERROR: JWT_REFRESH_SECRET environment variable MUST be set in production! ❌❌❌');
        process.exit(1);
    }
...
class SQLiteConnectionPool {
    ...
    async initialize() {
        ...
        for (let i = 0; i < this.maxConnections; i++) {
            ...
            // Her bağlantı için performans optimizasyonları - ULTRA HIZLI
            await conn.exec(`
...
async function initializeDatabase() {
    try {
        ...
        await rawDb.exec(`
            ...
            -- ✅ YENİ: Post görünürlük yönetimi
            CREATE TABLE IF NOT EXISTS post_visibility (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                postId TEXT NOT NULL UNIQUE,
                creatorId TEXT NOT NULL,
                maxImpressions INTEGER DEFAULT 100,
                currentImpressions INTEGER DEFAULT 0,
                isViralThresholdReached INTEGER DEFAULT 0,
                createdAt TEXT NOT NULL,
                FOREIGN KEY (postId) REFERENCES posts(id) ON DELETE CASCADE,
                FOREIGN KEY (creatorId) REFERENCES users(id) ON DELETE CASCADE
            );
            CREATE INDEX IF NOT EXISTS idx_post_visibility_postId ON post_visibility(postId);
            CREATE INDEX IF NOT EXISTS idx_post_visibility_creatorId ON post_visibility(creatorId);
...
const fileFilter = (req, file, cb) => {
    ...
    if (safeName.includes('..') || safeName.includes('/') || safeName.includes('\\')) {
        console.warn(`⚠️ Güvensiz dosya adı: ${file.originalname}`);
        return cb(new Error('Güvensiz dosya adı tespit edildi'), false);
    }
    
    // 🔒 Null byte injection kontrolü
    if (file.originalname.includes('\x00')) {
        console.warn(`⚠️ Null byte injection tespit edildi`);
        return cb(new Error('Güvenlik ihlali tespit edildi'), false);
    }
    ...
    // 🔒 Executable dosya uzantıları ekstra kontrol
    const executableExtensions = ['.exe', '.bat', '.cmd', '.sh', '.php', '.jsp', '.asp', '.aspx', '.js', '.jar', '.war', '.ear', '.py', '.rb', '.pl'];
    if (executableExtensions.includes(ext)) {
        console.warn(`⚠️ Executable dosya engellendi: ${ext}`);
        return cb(new Error('Çalıştırılabilir dosyalar kabul edilmez'), false);
    }
...
// 🔒 Token Blacklist temizleme (her saat expired token'ları temizle)
setInterval(() => {
...
const authenticateToken = async (req, res, next) => {
    ...
    if (tokenBlacklist.has(token)) {
        return res.status(401).json({ error: 'Token geçersiz kılınmış (logout)' });
    }
    ...
    try {
        ...
        // 🔒 Token issued date kontrolü (çok eski token'lar geçersiz)
        if (decoded.iat && (now - decoded.iat) > (90 * 24 * 3600)) { // 90 gün
        ...
        const user = await db.get('SELECT * FROM users WHERE id = ? AND isActive = 1', decoded.id);
        if (!user) {
            return res.status(403).json({ error: 'Kullanıcı bulunamadı veya hesap devre dışı' });
        }
        
        // 🔒 Password değişikliği kontrolü (eğer varsa)
        if (decoded.passwordVersion && user.passwordVersion && decoded.passwordVersion !== user.passwordVersion) {
            return res.status(401).json({ error: 'Şifre değiştirilmiş, yeniden giriş yapın' });
        }
        ...
        // 🔒 Token'ı request'e ekle (logout için)
        req.token = token;
...
// SQL Injection pattern'leri - ✅ GÜÇLENDİRİLDİ v2
    ...
    try {
        ...
        const posts = await db.all(
            `SELECT 
                ...
                pv.maxImpressions,
                pv.currentImpressions,
                pv.isViralThresholdReached,
                CASE 
                    WHEN EXISTS(SELECT 1 FROM likes WHERE postId = p.id AND userId = ?) THEN 1
                    ELSE 0
                END as isLiked,
                ...
                END as isSaved,
                CASE 
                    WHEN p.userId = ? THEN 1
                    ELSE 0
                END as isOwnPost,
                CASE
                    WHEN u.isVerified = 1 THEN 10
                    WHEN pv.isViralThresholdReached = 1 THEN 5
                    WHEN RANDOM() % 10 = 0 THEN 3
                    ELSE 1
                END as displayBoost
             FROM posts p 
             JOIN users u ON p.userId = u.id
             LEFT JOIN post_visibility pv ON p.id = pv.postId
             WHERE p.isActive = 1 AND u.isActive = 1
               AND (
                 p.userId = ?
                 OR u.isVerified = 1
                 OR pv.isViralThresholdReached = 1
                 OR (pv.currentImpressions < pv.maxImpressions AND RANDOM() % 3 = 0)
                 OR (EXISTS(SELECT 1 FROM follows WHERE followerId = ? AND followingId = p.userId))
               )
             ORDER BY 
                isOwnPost DESC,
                u.isVerified DESC,
                ...
                p.createdAt DESC
             LIMIT ? OFFSET ?`,
            req.user.id, req.user.id, req.user.id, req.user.id, req.user.id, limitNum, offset
        );
        
        // İmpressions güncelle (sadece başkasının postları için)
        for (const post of posts) {
            if (post.userId !== req.user.id && !post.isOwnPost) {
                await db.run(
                    `UPDATE post_visibility 
                     SET currentImpressions = currentImpressions + 1,
                         isViralThresholdReached = CASE 
                            WHEN currentImpressions + 1 >= maxImpressions * 0.8 THEN 1 
                            ELSE isViralThresholdReached 
                         END
                     WHERE postId = ? AND currentImpressions < maxImpressions`,
                    post.id
                ).catch(() => {}); // Silent fail
            }
        }
...
`);
    ...
    try {
        ...
        try {
            ...
            // ✅ Post görünürlük kaydı oluştur
            const isVerified = user.isVerified === 1;
            const maxImpressions = isVerified ? 10000 : 100; // Verified 10k, normal 100 kişiye gösterilir
            
            await db.run(
                `INSERT INTO post_visibility (postId, creatorId, maxImpressions, currentImpressions, createdAt)
                 VALUES (?, ?, ?, 0, ?)`,
                postId, user.id, maxImpressions, now
            );
            
            console.log(`✅ [ADIM 8.1] Görünürlük kaydı oluşturuldu (max: ${maxImpressions})\n`);
